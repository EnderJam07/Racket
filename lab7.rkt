...
(define (build-new-pairs x yz-pairs)
  (cond
    [(null? yz-pairs) '()]
    [else (cons (list x (cadar yz-pairs)) (build-new-pairs x (cdr yz-pairs)))]))
;1.
...
(define (Transitive-Closure L [full L] [original-full full])
  (cond
    ...
    [else (Transitive-Closure (cdr L) (remove-duplicates (append full (build-new-pairs (caar L) (find-connections (cadar L) full)))) original-full)]))